...
interface Props {
  onImport: (teachers: CsvTeacher[]) => Promise<{ message?: string } | null>;
}
...
export default function TeacherCsvImport({ onImport }: Props) {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (dateIdx !== -1 && cols[dateIdx]?.trim()) {
          ...
          if (!Number.isNaN(parsedDate.getTime())) {
            joiningDate = parsedDate.toISOString().slice(0, 10);
          }
        }
        ...
        if (Number.isNaN(monthlySalary) || monthlySalary < 0) {
          rowErrors.push(`Row ${i + 1}: Invalid salary "${salaryRaw}", skipped.`);
          continue;
        }
        ...
        teachers.push({
          ...
          joiningDate,
          status: statusIdx !== -1 && cols[statusIdx]?.trim().toLowerCase() === "inactive" ? "inactive" : "active",
          monthlySalary,
        });
      }
  ...
  const handleImport = async () => {
    ...
    if (!err) {
      ...
      setErrors((prev) => [...prev, `Import error: ${err.message ?? "Unknown error"}`]);
    }
  };
            ...
            <>
              ...
              <div className="border rounded max-h-60 overflow-y-auto">
                <Table>
                  ...
                  <TableBody>
                    {parsed.map((teacher, index) => (
                      <TableRow key={index}>
                        <TableCell>{teacher.name}</TableCell>
                        <TableCell>{teacher.contact || "-"}</TableCell>
                        <TableCell>{teacher.cnic || "-"}</TableCell>
                        <TableCell>{formatPKR(teacher.monthlySalary)}</TableCell>
